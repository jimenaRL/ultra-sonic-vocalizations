...
def create_dataframe(experiment):
    df = pd.read_csv(get_csv_path(experiment), header=0, na_values=0, keep_default_na=False)[COLUMNS]
...
def compute_mfcc(melspectrogram):
    tmp = librosa.feature.mfcc(S=librosa.power_to_db(melspectrogram), **MFCCPAMARS)
...
def get_dataframe():
    ...
    df = pd.concat([create_dataframe(e) for e in EXPERIMENTS]).rename({"sample": "sample_nb"}, axis=1)
    ...
    df = df.assign(duration=df['t1'].astype(np.float)-df['t0'].astype(np.float))
...
def get_dataset(recompute=False, element_spec=False):
    ...
    C = 1000
    dataset = dataset.map(lambda sample: dict(sample, stft=tf.math.log(1+C*sample["stft"])))
    dataset = dataset.map(lambda sample: dict(sample, mel=tf.math.log(1+C*sample["mel"])))

    # compute time aggregated features 
    dataset = dataset.map(lambda sample: dict(sample, mean_stft=reduce_time_mean((sample['stft']))))
    dataset = dataset.map(lambda sample: dict(sample, mean_mel=reduce_time_mean((sample['mel']))))
    dataset = dataset.map(lambda sample: dict(sample, mean_mfcc=reduce_time_mean((sample['mfcc']))))

    dataset = dataset.map(lambda sample: dict(sample, inv_stft=inverse((sample['stft']))))
    dataset = dataset.map(lambda sample: dict(sample, inv_mel=inverse((sample['mel']))))
    dataset = dataset.map(lambda sample: dict(sample, inv_mfcc=inverse((sample['mfcc']))))

    dataset = dataset.map(lambda sample: dict(sample, max_stft=reduce_time_max((sample['inv_stft']))))
    dataset = dataset.map(lambda sample: dict(sample, max_mel=reduce_time_max((sample['inv_mel']))))
    dataset = dataset.map(lambda sample: dict(sample, max_mfcc=reduce_time_max((sample['inv_mfcc']))))
    ...
    if recompute:
        try:
            ...
            os.mkdir(CACHEDIR)
            print(f"Directory {CACHEDIR} has been removed and recreated successfully.") 
